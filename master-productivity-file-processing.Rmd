...
cpsc_data <- read_excel(paste0(path, "cpsc_data_sample_clean.xlsx"))
benchmark_data <- read_excel(paste0(path, "cpsc_benchmarks_sample.xlsx"))
collections_data <- read_excel(paste0(path, "collections_data_sample_clean.xlsx"))
...
```{r Map Additional Provider Details, echo = FALSE, warning = FALSE, message = FALSE}
...
ytd_productivity_output <- cpsc_data_master %>% 
  arrange(year_month) %>%
  group_by(PROVIDER, NPI, Specialty, Year) %>%
  summarise(payments_per_WRVU_ytd = payments_per_WRVU_ytd[which.max(Month)],
            productivity_ytd = productivity_ytd[which.max(Month)]) %>%
  group_by(PROVIDER, NPI, Specialty) %>%
  pivot_wider(names_from = Year,
              values_from = payments_per_WRVU_ytd:productivity_ytd) 
...
addStyle(wb, 1, provStyle, rows = 3, cols = 1:3)

#### Monthly Data Section
addStyle(wb, 1, wRVUsStyle, rows = 3, cols = 4:(3+length(month_col_names)))
addStyle(wb, 1, number_format, rows = 4:(3+nrow(productivity_output)), cols = 4:(3+length(month_col_names)), gridExpand = TRUE)

addStyle(wb, 1, collectionsStyle, rows = 3, cols = (4+length(month_col_names)):(3+(length(month_col_names)*2)))
addStyle(wb, 1, dollar_format, rows = 4:(3+nrow(productivity_output)), cols = (4+length(month_col_names)):(3+(length(month_col_names)*2)), gridExpand = TRUE)

addStyle(wb, 1, cfteStyle, rows = 3, cols = (4+(length(month_col_names)*2)):(3+(length(month_col_names)*3)))
addStyle(wb, 1, decimal_format, rows = 4:(3+nrow(productivity_output)), cols = (4+(length(month_col_names)*2)):(3+(length(month_col_names)*3)), gridExpand = TRUE)

addStyle(wb, 1, productivityStyle, rows = 3, cols = (4+(length(month_col_names)*3)):(3+(length(month_col_names)*4)))

#### Yearly Data Section
addStyle(wb, 1, wRVUsStyle, rows = 3, cols = (4+(length(month_col_names)*4)):(3+(length(month_col_names)*4)+(length(year_col_names))))
addStyle(wb, 1, number_format, rows = 4:(3+nrow(productivity_output)), cols = (4+(length(month_col_names)*4)):(3+(length(month_col_names)*4)+(length(year_col_names))), gridExpand = TRUE)

addStyle(wb, 1, collectionsStyle, rows = 3, cols = (4+(length(month_col_names)*4)+(length(year_col_names))):(3+(length(month_col_names)*4)+(length(year_col_names)*2)))
addStyle(wb, 1, dollar_format, rows = 4:(3+nrow(productivity_output)), cols = (4+(length(month_col_names)*4)+(length(year_col_names))):(3+(length(month_col_names)*4)+(length(year_col_names)*2)), gridExpand = TRUE)

addStyle(wb, 1, cfteStyle, rows = 3, cols = (4+(length(month_col_names)*4)+(length(year_col_names))*2):(3+(length(month_col_names)*4)+(length(year_col_names)*3)))
addStyle(wb, 1, decimal_format, rows = 4:(3+nrow(productivity_output)), cols = (4+(length(month_col_names)*4)+(length(year_col_names))*2):(3+(length(month_col_names)*4)+(length(year_col_names)*3)), gridExpand = TRUE)

addStyle(wb, 1, varianceStyle, rows = 3, cols = (4+(length(month_col_names)*4+(length(year_col_names)*3))):(3+(length(month_col_names)*4)+(length(year_col_names)*5)))
addStyle(wb, 1, number_format, rows = 4:(3+nrow(productivity_output)), cols = (4+(length(month_col_names)*4+(length(year_col_names)*3))):(4+(length(month_col_names)*4+(length(year_col_names)*3))), gridExpand = TRUE)
addStyle(wb, 1, percent_format, rows = 4:(3+nrow(productivity_output)), cols = (4+(length(month_col_names)*4+(length(year_col_names)*3))+1):(4+(length(month_col_names)*4+(length(year_col_names)*3))+1), gridExpand = TRUE)
addStyle(wb, 1, dollar_format, rows = 4:(3+nrow(productivity_output)), cols = (4+(length(month_col_names)*4+(length(year_col_names)*3))+2):(4+(length(month_col_names)*4+(length(year_col_names)*3))+2), gridExpand = TRUE)
addStyle(wb, 1, percent_format, rows = 4:(3+nrow(productivity_output)), cols = (4+(length(month_col_names)*4+(length(year_col_names)*3))+3):(4+(length(month_col_names)*4+(length(year_col_names)*3))+3), gridExpand = TRUE)

addStyle(wb, 1, perwRVUStyle, rows = 3, cols = (4+(length(month_col_names)*4+(length(year_col_names)*5))):(3+(length(month_col_names)*4)+(length(year_col_names)*6)))
addStyle(wb, 1, dollar_format, rows = 4:(3+nrow(productivity_output)), cols = (4+(length(month_col_names)*4+(length(year_col_names)*5))):(3+(length(month_col_names)*4)+(length(year_col_names)*6)), gridExpand = TRUE)

addStyle(wb, 1, productivityStyle, rows = 3, cols = (4+(length(month_col_names)*4+(length(year_col_names)*6))):(3+(length(month_col_names)*4)+(length(year_col_names)*7)))


#### Table Formatting 
setColWidths(wb, 1, cols = c(1,3), widths = 40)
setColWidths(wb, 1, cols = 4:length(productivity_output), widths = 14)

freezePane(wb, 1, firstActiveRow = 4, firstActiveCol = 4)
...
library(writexl)
write_xlsx(cpsc_data_master, paste0(getwd(),"/R Processed Productivity Data.xlsx"))
